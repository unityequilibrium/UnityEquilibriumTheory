"""Audit the same-kernel tree tagged and spectral width."""
...
def scale_witness(scale: float) -> dict[str, object]:
    ...
    ratios = (
        np.asarray(scaled.total_widths_by_tag_momentum)[:, 0]
        / np.asarray(base.total_widths_by_tag_momentum)[:, 0]
    )
    return {
        "scale": scale,
...
def convergence_witnesses() -> dict[str, object]:
    ...
    for order in (12, 16, 24):
        ...
        radial_rows.append({
            "radial_order": order,
            "widths_by_tag": np.asarray(state.total_widths_by_tag_momentum)[:, 0].tolist(),
        })
    angular_rows = []
    for order in (4, 6, 8):
        state = same_kernel_tagged_width_state(
    ...
    return {
        ...
        "radial_last_relative_difference": _relative_array(
            np.asarray(radial_rows[-2]["widths_by_tag"]),
            np.asarray(radial_rows[-1]["widths_by_tag"]),
        ),
        "angular_last_relative_difference": _relative_array(
            np.asarray(angular_rows[-2]["widths_by_tag"]),
            np.asarray(angular_rows[-1]["widths_by_tag"]),
        ),
    }
...
def main() -> int:
    ...
    widths = np.asarray(reference.total_widths_by_tag_momentum)
    spectral = np.asarray(reference.retarded_self_energy_imaginary_by_tag_momentum)
    ...
    checks = {
        ...
        "positive_momentum_resolved_widths": bool(np.all(widths > 0.0)),
        "spectral_interface_exact": bool(np.allclose(spectral, -2.0 * energies[None, :] * widths, rtol=1.0e-14)),
    ...
    artifact = {
        "schema_version": "t13-same-kernel-tagged-width-v1",
        "major_result_id": "T13_SAME_KERNEL_TREE_TAGGED_SPECTRAL_WIDTH",
        "topic": "0.13_Thermodynamic_Bridge",
        "closure_level": "CLOSED_FOR_LANE",
        "verification_status": status,
        "what_is_closed": [
            "Charge- and momentum-resolved tagged elastic loss widths are derived from the same production contact-plus-Phi amplitude and center-of-mass event kernel as the invariant Galerkin operator.",
            ...
            "The on-shell spectral interface -Im Sigma_R=2E Gamma is explicit, dimensionally covariant and uses no fitted damping input.",
        ],
        ...
        "derivation_class": "same_action_tree_elastic_cut_width_with_independent_cross_section_match",
        "observable": "Tagged elastic quasiparticle loss and on-shell spectral width, not conductivity",
        ...
        "claim_boundary": "Tree elastic same-kernel tagged and spectral width only; not a dressed self-consistent/resonant width, complete damping rate, vector heat-current ladder, Kubo/SI coefficient, external validation, or Full Topic 13 closure.",
    }
    artifact["report"] = {
        ...
        "STATUS": status,
        "WHAT_CHANGED": "Derived tagged and spectral widths from the same invariant production kernel and matched an independent cross-section form.",
    ...
    entry.update({
        ...
        "version": "1",
        "classification": "candidate_same_kernel_tree_tagged_spectral_width",
        "relation_or_code_path": artifact["equation_or_mapping"],
        "standard_physics_counterpart": "Relativistic tagged 2-to-2 loss rate and on-shell retarded self-energy width",
        "variables": {"Gamma_q": "tagged elastic loss width", "Sigma_R": "retarded on-shell self-energy"},
        ...
        "proof_status": "TREE_TAGGED_WIDTH_AND_INDEPENDENT_NORMALIZATION_VERIFIED",
