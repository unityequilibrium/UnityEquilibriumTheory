...
def test_tpg_alpha_v_lane_is_integrated_without_full_bridge_promotion() -> None:
    lane = load(LANE)
    full = load(FULL)
    projected = full["verification_status"]["source_package"][
        "tpg_anisotropic_alpha_v_comparator"
    ]
    assert projected["major_result_id"] == "T13_TPG_ANISOTROPIC_ALPHA_V_COMPARATOR"
    ...
    assert projected["derived_comparator"]["alpha_V_per_K"] == 2.476e-5
    assert projected["derived_comparator"]["same_specimen_alpha_V"] is False
    ...
    assert any(
        item["path"] == "docs/core/07_artifacts/topic13/t13_tpg_anisotropic_alpha_v_source_audit.json"
