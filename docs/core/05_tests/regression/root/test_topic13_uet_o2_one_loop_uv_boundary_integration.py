...
def test_uv_boundary_is_integrated_without_unlock() -> None:
    full = load(FULL)
    register = load(REGISTER)
    dependency = load(DEPENDENCY)
    result_id = "T13_UET_O2_ONE_LOOP_THERMAL_UV_BOUNDARY"
    assert full["verification_status"]["eos_transport_kms_entropy"]["uet_o2_one_loop_uv_boundary"]["major_result_id"] == result_id
    assert any(item.get("major_result_id") == result_id for item in register["entries"])
    partial = dependency["topic13_partial_evidence"]
    assert partial["uet_o2_one_loop_uv_boundary"]["summary"]["major_result_id"] == result_id
    assert partial["uet_o2_one_loop_uv_boundary_full_core_unlock"] is False
