...
def test_finite_temperature_scattering_channel_has_positive_cut_and_negative_retarded_sign():
...
def test_finite_temperature_scattering_channel_matches_kms_fdt_and_converges():
    state = finite_temperature_scattering_sunset_sk_kms_state(0.35, 0.5, 0.8)
    assert state.kms_log_ratio_residual <= 2.0e-2
    assert state.fdt_residual <= 2.0e-2
    assert state.scattering_inner_convergence_residual <= 2.0e-2
    assert state.scattering_outer_convergence_residual <= 2.0e-2
    assert state.finite_temperature_principal_value_completed
    assert state.finite_temperature_principal_value_real_part != 0.0
    assert state.scattering_pv_inner_convergence_residual <= 2.0e-2
    assert state.scattering_pv_outer_convergence_residual <= 2.0e-2


def test_finite_temperature_scattering_contract_keeps_full_closure_open():
    contract = finite_temperature_scattering_sunset_sk_kms_contract()
    assert contract["included"]["labeled_finite_temperature_scattering_cut"]
    assert contract["included"]["channel_level_sk_kms_match"]
    assert contract["included"]["pole_subtracted_channel_real_part"]
    assert contract["excluded"]["other_finite_temperature_sunset_cuts"]
