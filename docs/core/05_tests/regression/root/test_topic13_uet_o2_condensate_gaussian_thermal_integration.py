...
def test_gaussian_lane_is_integrated_without_downstream_unlock() -> None:
    result_id = "T13_UET_O2_CONDENSATE_GAUSSIAN_FINITE_T_LANE"
    full = load(FULL)
    register = load(REGISTER)
    dependency = load(DEPENDENCY)
    lane = full["verification_status"]["eos_transport_kms_entropy"]["uet_o2_condensate_gaussian_finite_t_lane"]
    assert lane["major_result_id"] == result_id
    assert lane["closure_level"] == "CLOSED_FOR_LANE"
    assert lane["fixed_tree_level_background"] is True
    assert lane["normal_two_fluid_completion"] is False
    ...
    assert partial["uet_o2_condensate_gaussian_finite_t_lane_full_core_unlock"] is False
